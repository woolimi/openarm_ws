...
def parse_args(argv):
    parser = argparse.ArgumentParser(
        prog='register',
        description='Feetech 리더 서보 id 등록. 인자 없이 실행하면 버스를 스캔만 합니다.')
    parser.add_argument(
        '--port', default='/dev/ttyUSB0',
        help='리더암 시리얼 포트 (기본 /dev/ttyUSB0)')
    parser.add_argument(
        '--id', type=int, dest='new_id',
        help='배정할 id (1~253). 생략하면 스캔 결과만 보여줍니다.')
    parser.add_argument(
        '--from', type=int, dest='current_id',
        help='바꿀 서보의 현재 id. 버스에 서보가 하나뿐이면 생략합니다.')
    parser.add_argument(
        '--full-scan', action='store_true',
        help=f'id 1~{SCAN_MAX_ID_FULL} 전체를 스캔합니다 (기본 1~{SCAN_MAX_ID_DEFAULT})')
    parser.add_argument(
        '--config',
        help='feetech 버스 설정을 읽을 leader.yaml 경로. 생략하면 설치본을 씁니다.')
...
def main(argv=None):
    ...
    if args.new_id is not None and not 1 <= args.new_id <= SCAN_MAX_ID_FULL:
        sys.exit(f'id 는 1~{SCAN_MAX_ID_FULL} 범위여야 합니다: {args.new_id}')
    ...
    try:
        ...
        if not found:
            sys.exit('응답하는 서보가 없습니다. 전원·배선·baudrate 를 확인해 주세요.')
        ...
        if args.current_id is not None:
            ...
            sys.exit('서보가 여러 개 잡혔습니다. --from 으로 바꿀 서보의 현재 id 를 지정해 주세요.')

        if current_id == args.new_id:
            print(f'서보가 이미 id {args.new_id} 입니다.')
            return
        if args.new_id in found:
            sys.exit(f'id {args.new_id} 는 버스의 다른 서보가 이미 쓰고 있습니다.')

        if not bus.set_servo_id(current_id, args.new_id):
            sys.exit(f'id {current_id} → {args.new_id} 변경에 실패했습니다.')
