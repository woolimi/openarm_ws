...
Enter 를 누르면 다음 팔로 넘어간다. 영점 자세는 팔로워의 영점(모든 관절이
0 인 자세)과 같은 자세다 — 팔로워를 그 자세로 두고 리더암을 그 모양에 맞춘다. 한 팔만 다시 잡으려면 `--arm left` 처럼 지정한다.
...
팔로워를 실기로 띄운다. 활성화 과정에서 양팔이 **2초에 걸쳐 영점 자세로 이동**하는데, 이동
시간이 고정이라 시작 위치가 멀수록 빠르게 움직인다. 띄우기 전에 두 팔을 손으로 영점 자세
근처에 옮겨 두고 팔 주변 공간을 비운다.
