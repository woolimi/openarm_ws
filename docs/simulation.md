...
하강 경로의 달성률이 `100.0%` 로 찍히며, 예제 04 는 그리퍼가 `0.035 m` 로 열렸다 `0.000 m` 로
닫힌다.
...
작은 창이 하나 뜬다. 키 입력은 그 창으로 받는다 — 터미널은 키를 뗀 순간을 알려주지 않아
"얼마간 입력이 없으면 뗀 것" 으로 추측해야 하고, 그러면 키를 꾹 누를 때 팔이 한 번 멈칫한다.
창은 지금 눌려 있는 키를 그대로 알려주므로 멈칫이 없고, 여러 키를 같이 눌러 대각선으로도
움직일 수 있다.

**확인** — 팔이 ready 자세로 간 뒤 창에 `Servo READY` 가 뜬다. `w/s`·`a/d`·`q/e` 를 누르는 동안
손끝이 x·y·z 로, Shift 를 같이 누르면 roll·pitch·yaw 로 움직인다. `w` 와 `a` 를 같이 누르면
대각선으로 간다. 창 밖을 클릭하면 명령이 멈추고, 다시 창을 클릭하면 이어진다. 관절 한계나
특이자세에서 상태가 `정지` 로 바뀌면 `r` 로 ready 에 돌아온다. `space` 는 즉시 정지, `ESC` 로
끝낸다.
